# locals 块在 Terraform 中用于定义局部变量，这些变量不会暴露给模块使用者，仅在模块内部使用。当前
locals {
...
#    按顺序包括 namespace、environment、stage、name 和 attributes，这些是常见的标识符组成部分。
#    例如，如果定义一个虚拟机的资源名称，可以基于此顺序生成名字：dev-production-stage-webapp.
    label_order         = ["namespace", "environment", "stage", "name", "attributes"]
#    定义了正则表达式，用于替换不合法的字符。确保标签或标识符符合标准格式（如只能包含字母、数字和连接符）。
    regex_replace_chars = "/[^-a-zA-Z0-9]/"
#    分隔符，通常用于拼接字符串
    delimiter           = "-"
#    指定替换不合法字符时使用的字符串。默认为空字符串，表示直接移除不合法字符。
    replacement         = ""
    id_length_limit     = 0
    id_hash_length      = 5
...
#    这个表达式动态生成 attributes 的值，通过合并多个输入变量的方式生成一个去重且无空值的数组
