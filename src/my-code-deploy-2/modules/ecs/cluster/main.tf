...
resource "aws_ecs_cluster" "this" {
  ...
  dynamic "configuration" {
    ...
    content {
#      execute_command_configuration 是一个可选的配置，只有在你需要在 ECS 容器中执行命令时才需要启用。如果你的工作负载不需要这个功能，完全可以省略它，这不会影响 ECS 集群的正常创建和运行。
      dynamic "execute_command_configuration" {
...
#  service_connect_defaults 这个配置是 service级别的
...
################################################################################
# CloudWatch Log Group 默认创建 aws_cloudwatch_log_group
################################################################################
...
# 在你创建好ecs cluster的时候， 你可以更新cluster的 capacity provider
# 如果你想创建capacity provider 你需要传入 fargate_capacity_providers 或者 autoscaling_capacity_providers
...
################################################################################
# 默认不创建， 如果你需要创建，则需要传入 autoscaling_capacity_providers
...
# create_task_exec_iam_role 默认不创建， 如果你需要创建，则需要传入 create_task_exec_iam_role
...
# 默认不创建，如果你需要创建则需要create_task_exec_policy 参数
